...
ANNOT_PATH = '/home/aidara/Desktop/Thesis_Andrea/data/annotations_POV-Surgey_object_False_NEW'
...
name = 'images'
for split in SPLITS:
    file_path = os.path.join(OLD_ANNOT_PATH, f'{name}-{split}.{EXTENSION}')
    IMAGES[split] = np.load(file_path, allow_pickle=True)
    
# Remove bbox=None images and
# Bounding boxes
name = 'boxes'
for split in SPLITS:
    ...
    for i, fp in enumerate(images_split):
        try:
            ...
            if bbox != [None] * 4:
                images.append(fp)
                boxes.append(np.array(bbox) / np.array([IMG_SIZE[0], IMG_SIZE[1], IMG_SIZE[0], IMG_SIZE[1]]))  # Normalized bbox
...
for split in SPLITS:
    ...
    IMAGES_NEW[split] = np.load(file_path, allow_pickle=True)



# -------------- Created .npy files for Keypoints2d, Keypoints3d and Mesh3d --------------
